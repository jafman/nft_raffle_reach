...
export const main = Reach.App(() => {
  ...
  A.only(()=>{
    ...
    const _winningNum = interact.getNum(numTickets); // not declassified
  ...
  commit();
  A.pay([[amt, nftId]]); // cos its a non network token
  commit();
  ...
  B.interact.seeWinner(winningNum); // shorthand for B.only() step
